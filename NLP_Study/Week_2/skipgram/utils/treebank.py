...
class RTE_dataset:
    ...
    def tokens(self):
        ...
        #_revtokens 와 tokens 의 길이가 같아야 함
        assert len(revtokens) == len(tokens), "Length of revtokens and tokens must be equal."
    ...
    def sampleTokenIdx(self, outsideWordIdx, K):
        ...
        if not (hasattr(self, "_samplingFreq") and self._samplingFreq is not None):
            ...
            nTokens = len(self._revtokens) #GPT를 따라 수정함# initialize self._tokenfreq, self._tokens, self._revtokens and get num tokens
        ...
        idx = None
        #######Your Code#######
        idx = np.random.choice(len(tmpSamplingFreq), size=K, p=tmpSamplingFreq)
        #######################
        return idx
