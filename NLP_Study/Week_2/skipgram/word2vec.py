...
def naiveSoftmaxLossAndGradient(
    ...
    loss = -np.log(y_hat[outsideWordIdx])  # Loss is the negative log likelihood
    gradCenterVec = -outsideVectors[outsideWordIdx] + y_hat.dot(outsideVectors)  # Gradient w.r.t. center word vector
    y_hat[outsideWordIdx] -= 1  # Adjust the prediction for the outside word
    gradOutsideVecs = np.outer(y_hat, centerWordVec)  # Gradient w.r.t. outside vectors
...
def negSamplingLossAndGradient(
    ...
    if hasattr(dataset, "_samplingFreq"):
        assert len(dataset._samplingFreq) == outsideVectors.shape[0], \
            f"[ERROR] dataset._samplingFreq({len(dataset._samplingFreq)}) ≠ outsideVectors.shape[0] ({outsideVectors.shape[0]})"
    negSampleVectors = outsideVectors[indices]  # Get the vectors for the outside word and negative samples
    negSampleScores = np.dot(negSampleVectors, centerWordVec) 
    negSampleScores = sigmoid(negSampleScores)

    eps = 1e-10  # To avoid log(0)
    loss = -np.log(np.clip(negSampleScores[0], eps, 1.0)) - np.sum(np.log(np.clip(1 - negSampleScores[1:], eps, 1.0)))
    gradCenterVec = (negSampleScores[0] - 1) * negSampleVectors[0] + np.sum(negSampleScores[1:, np.newaxis] * negSampleVectors[1:], axis=0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (negSampleScores[0] - 1) * centerWordVec
    for i, idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[idx] += negSampleScores[i + 1] * centerWordVec
...
def getDummyObjects():
    ...
    dataset = type('dummy', (), {})()
    dataset.sampleTokenIdx = lambda o, k: [random.randint(0, 4) for _ in range(k)]
